            ...
            if (args.length == 0) {
                ...
                if (t instanceof Player) {
                    if (plugin.dead_players.contains(p)) {
                        p.sendMessage(ChatColor.translateAlternateColorCodes('&', Config.get().getString("prefix") + "&e"+t.getDisplayName()+" &fis currently dead!"));
                    } else {
                        p.sendMessage(ChatColor.translateAlternateColorCodes('&', Config.get().getString("prefix") + "&e"+t.getDisplayName()+" &fis currently alive!"));
                    }
                }
            }
        }
